...
class QuickstartChatManager: NSObject, TwilioChatClientDelegate {
    ...
    var uniqueChannelName = "channel#123"
    var friendlyChannelName = "channel#123"
    ...
    private var messages1:Set<TCHMessage> = Set<TCHMessage>()
    ...
    private func refreshAccessToken() {
        ...
        let urlString = "\(TOKEN_URL)"

        TokenUtils.retrieveToken(url: urlString, identity: identity) { (token, _, error) in
    ...
    func login(_ identity: String, completion: @escaping (Bool) -> Void) {
        ...
        TokenUtils.retrieveToken(url: urlString.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)!, identity: identity) { (token, _, error) in
            guard let token = token else {
                print("Error retrieving token: \(error.debugDescription)")
                completion(false)
                return
    ...
    private func joinChannel(_ channel: TCHChannel) {
        ...
        if channel.status == .joined {
            AppDelegate.shared.chatClient = client
            AppDelegate.shared.registerRemoteNotifications() // called appdelegate method
            print("Current user already exists in channel")
            if self.channel?.synchronizationStatus == .all {
                self.channel?.messages?.getLastWithCount(100) { (result, items) in
            ...
            channel.join(completion: { result in
                ...
                AppDelegate.shared.chatClient = self.client
                AppDelegate.shared.registerRemoteNotifications() // called appdelegate method
    ...
    func addMessages(newMessages:Set<TCHMessage>) {
        messages1 =  messages1.union(newMessages)
        sortMessages()
        DispatchQueue.main.async {
    ...
    func sortMessages() {
        messages = messages1.sorted { (a, b) -> Bool in
