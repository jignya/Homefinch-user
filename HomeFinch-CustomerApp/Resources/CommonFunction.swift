...
//
//  Created by Jignya Panchal on 24/11/20.
//
...
class CommonFunction: NSObject {
    ...
    func GetVideoUrl(Filename:String) -> URL
    {
        ...
        let videoDataPath = documentsDirectory + "/" + Filename
    ...
    func GetImagePath(Filename:String) -> String
    {
        let paths = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true)
        let documentsDirectory = paths[0]
        let imgDataPath = documentsDirectory + "/" + Filename
//        let filePathURL = URL(fileURLWithPath: videoDataPath)
        return imgDataPath
    }
    ...
    func clearFilesFromDirectory(filename:String){
    ...
    func SaveDictionaryDatainTextFile(fileName:String , arrData:[String:Any])
    {
        var jsonData: NSData!
        do
        {
    ...
    //MARK: setIssueStatusIcon
    
    func setIssueStatusIcon(status:Int) -> UIImage?
    {
        if status == 1 || status == 2 || status == 3
        {
            return UIImage(named: "pending1")
        }
        else if status == 4 || status == 5 || status == 6 || status == 7 || status == 10 || status == 12 || status == 14
        {
            return UIImage(named: "inprocess1")
        }
        else if status == 8 || status == 11
        {
            return UIImage(named: "cancelled1")
        }
        else if status == 13
        {
            return UIImage(named: "onhold1")
        }
        else
        {
            return UIImage(named: "completed1")
        }
    }
...
//            if (view is FloatLabelTextField)
//            {
//                let txt1 = view as! FloatLabelTextField
//
